<template>
  <div class="font-montserrat">
    ...
    >
      <h3
        class="mt-0 mb-6 text-[#D4D4D4] font-black uppercase tracking-widest text-sm border-b border-[#3B3B3B] pb-3"
      >
        Registrar Nuevo Entrenador
      </h3>

      <form
        @submit.prevent="guardarEntrenador"
      ...
      >
        ...
        <div class="flex flex-col">
          ...
          <input
            type="email"
            v-model="nuevo.email"
            placeholder="entrenador@lionsss.com"
            required
            class="w-full p-3 bg-[#222222] border border-[#3B3B3B] rounded-lg text-[#EEEEEE] placeholder-[#555555] focus:outline-none focus:border-[#D4D4D4] focus:ring-1 focus:ring-[#D4D4D4] transition-colors"
          />
        </div>
        ...
        <div>
          ...
          >
            <svg
              v-if="cargando"
              class="animate-spin h-5 w-5 text-[#222222]"
    ...
    <div
      class="bg-[#171717] rounded-xl overflow-hidden shadow-xl border border-[#3B3B3B]"
    >
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody>
            <tr
              ...
              :key="entrenador.id"
              class="hover:bg-[#222222]/50 transition duration-150 group"
            >
              <td class="p-4 border-b border-[#3B3B3B]">
                <div v-if="editandoId === entrenador.id">
                  <input
                    type="text"
                    v-model="entrenador.nombre_completo"
              ...
              <td
                class="p-4 border-b border-[#3B3B3B] text-[#888888] text-sm italic"
              >
                (Protegido por Supabase)
              </td>
              <td class="p-4 border-b border-[#3B3B3B]">
                <span
                  class="bg-[#3B3B3B] text-[#D4D4D4] px-3 py-1 rounded-full text-xs uppercase font-bold tracking-wide border border-[#555555]"
                >
                  {{ entrenador.rol }}
                </span>
              </td>
                ...
                <div class="flex justify-center gap-3">
                  <template v-if="editandoId === entrenador.id">
                    ...
                    <button
                      ...
                      @click="actualizarEntrenador(entrenador)"
                      class="text-green-500 hover:text-green-400 hover:scale-110 transition transform"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                    ...
                    <button
                      ...
                      class="text-red-500 hover:text-red-400 hover:scale-110 transition transform"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                  ...
                  <template v-else>
                    ...
                    <button
                      title="Editar"
                      @click="iniciarEdicion(entrenador)"
                      class="text-[#D4D4D4] hover:text-[#FAFAFA] hover:scale-110 transition transform"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                    ...
                    <button
                      title="Eliminar"
                      @click="eliminarEntrenador(entrenador.id)"
                      class="text-red-900 hover:text-red-500 hover:scale-110 transition transform"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
        ...
        >
          <div class="flex justify-between items-start">
            <div class="w-full">
              ...
              <h4 v-else class="text-lg font-bold text-[#EEEEEE] m-0">
                {{ entrenador.nombre_completo }}
              </h4>

              <p class="text-xs text-[#888888] mt-1 italic">
                Correo protegido por Supabase
              </p>
            </div>
          </div>
          ...
          <div class="flex justify-between items-center mt-2">
            <span
              class="bg-[#3B3B3B] text-[#D4D4D4] px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-wide border border-[#555555]"
            >
              {{ entrenador.rol }}
            </span>
            ...
            <div class="flex gap-4">
              <template v-if="editandoId === entrenador.id">
                <button
                  @click="actualizarEntrenador(entrenador)"
                  class="text-green-500 p-2 bg-[#222222] rounded-lg border border-[#3B3B3B]"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                ...
                <button
                  @click="editandoId = null"
                  class="text-red-500 p-2 bg-[#222222] rounded-lg border border-[#3B3B3B]"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
              ...
              <template v-else>
                <button
                  @click="iniciarEdicion(entrenador)"
                  class="text-[#D4D4D4] p-2 bg-[#222222] rounded-lg border border-[#3B3B3B]"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
