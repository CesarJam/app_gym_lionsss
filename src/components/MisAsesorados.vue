    ...
    <div v-if="!asesoradoSeleccionadoId">
      ...
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p class="text-[#888888] text-xs sm:text-sm mt-1 uppercase tracking-wider font-bold">
      ...
      <div v-if="mostrarFormulario" class="bg-[#171717] p-6 rounded-xl mb-8 border border-[#3B3B3B] shadow-2xl transition-all duration-300">
        <h3 class="mt-0 mb-2 text-[#D4D4D4] font-black uppercase tracking-widest text-sm">
          Alta Rápida
        </h3>
        <p class="text-xs text-[#888888] mb-5 border-b border-[#3B3B3B] pb-3">
          Solo necesitas su nombre. El sistema generará un enlace para que el cliente llene el resto de su información.
        </p>
        
        <form @submit.prevent="guardarAsesorado" class="flex flex-col sm:flex-row gap-4 items-end">
          <div class="flex flex-col flex-grow w-full">
            <label class="text-xs font-bold text-[#888888] mb-2 uppercase tracking-wide">Nombre Completo del Cliente</label>
          ...
          <div class="w-full sm:w-auto">
            <button type="submit" :disabled="cargando" 
                    class="w-full sm:w-auto bg-[#D4D4D4] hover:bg-[#FAFAFA] text-[#222222] font-extrabold uppercase tracking-wide py-3 px-6 rounded-lg transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2">
      ...
      <div class="bg-[#171717] rounded-xl shadow-xl border border-[#3B3B3B]">
        ...
        <div class="hidden md:block overflow-visible">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-[#222222]">
                <th class="text-[#888888] font-bold text-xs uppercase tracking-wider p-4 border-b border-[#3B3B3B] rounded-tl-xl">Nombre</th>
                <th class="text-[#888888] font-bold text-xs uppercase tracking-wider p-4 border-b border-[#3B3B3B]">Estado</th>
                <th class="text-[#888888] font-bold text-xs uppercase tracking-wider p-4 border-b border-[#3B3B3B]">Enlace de Registro</th>
                <th class="text-[#888888] font-bold text-xs uppercase tracking-wider p-4 border-b border-[#3B3B3B] text-center rounded-tr-xl">Acciones</th>
              </tr>
            </thead>
            <tbody>
              <tr v-if="asesorados.length === 0">
                <td colspan="4" class="p-8 text-center text-[#555555] font-bold uppercase tracking-widest">No tienes asesorados registrados aún.</td>
              </tr>
              <tr v-for="cliente in asesorados" :key="cliente.id" class="hover:bg-[#222222]/50 transition duration-150">
                <td class="p-4 border-b border-[#3B3B3B] text-[#EEEEEE] font-bold">
                  {{ cliente.nombre_completo }}
                </td>
                <td class="p-4 border-b border-[#3B3B3B]">
                  <span :class="[
                    'px-3 py-1 rounded-full text-[10px] uppercase font-black tracking-widest border',
                    cliente.estado_registro === 'completado' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'
                  ]">
                ...
                <td class="p-4 border-b border-[#3B3B3B]">
                  ...
                  <!-- Check Formulario -->
                  <span v-else class="text-xs font-bold uppercase tracking-wide text-[#888888] flex items-center gap-2">
                ...
                <td class="p-4 border-b border-[#3B3B3B] text-center relative">
                  ...
                  >
                    ...
                    >
                      <svg class="w-4 h-4 text-[#888888]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                      </svg> Ver Expediente
                    </button>
                    
                    <hr v-if="cliente.estado_registro === 'completado'" class="border-[#3B3B3B] my-1">
        ...
        <div class="md:hidden flex flex-col divide-y divide-[#3B3B3B]">
          <div v-if="asesorados.length === 0" class="p-8 text-center text-[#555555] font-bold uppercase tracking-widest text-sm">
            No tienes asesorados registrados aún.
          </div>
          <div v-for="cliente in asesorados" :key="'mob-' + cliente.id" class="p-4 flex flex-col gap-4 relative">
            
            <div class="flex justify-between items-start">
              <div>
                <h4 class="text-lg font-bold text-[#EEEEEE] m-0">{{ cliente.nombre_completo }}</h4>
                <div class="mt-2">
                  <span :class="[
              ...
              <div class="relative">
                <button @click="toggleMenu('mob-' + cliente.id)" class="text-[#888888] hover:text-[#FAFAFA] p-1 rounded-lg bg-[#222222] border border-[#3B3B3B]">
                  <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"></path>
                  </svg>
                </button>

                <div v-if="menuAbierto === 'mob-' + cliente.id" @click="cerrarMenu" class="fixed inset-0 z-10 cursor-default"></div>

                <div v-if="menuAbierto === 'mob-' + cliente.id" class="absolute right-0 top-10 w-48 bg-[#222222] border border-[#555555] rounded-xl shadow-2xl z-30 py-2 text-left overflow-hidden">
                  <button v-if="cliente.estado_registro === 'completado'" @click="abrirExpediente(cliente.id)" class="w-full text-left px-4 py-3 text-xs font-bold uppercase tracking-wide text-[#D4D4D4] hover:bg-[#3B3B3B] transition flex items-center gap-3">
                    <svg class="w-4 h-4 text-[#888888]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg> Ver Expediente
                  </button>
                  <hr v-if="cliente.estado_registro === 'completado'" class="border-[#3B3B3B] my-1">
                  <button @click="eliminarAsesorado(cliente.id)" class="w-full text-left px-4 py-3 text-xs font-bold uppercase tracking-wide text-red-500 hover:bg-[#3B3B3B] transition flex items-center gap-3">
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg> Eliminar
                  </button>
                </div>
              </div>
            </div>
            ...
            <div class="pt-2 border-t border-[#3B3B3B]">
              ...
              <span v-else class="text-xs font-bold uppercase tracking-wide text-[#888888] flex items-center justify-center gap-2 py-2">
                <svg class="w-5 h-5 text-green-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
